...
> **2026-08-30 迁移说明**：本报告原在仓库根 `docs/`，文件名为 `bakedmeshes-dedup-report.md`。
> 正文里的 `UnityProject/`、`Assets/MoMing/`、`FormalLevels/` 都是 **08-30 改名前的旧路径**，现已分别对应
> `Do not leave me/`、`Assets/DoNotLeaveMe/`、`Assets/DoNotLeaveMe/Levels/`。
> 同批迁移的还有 `BakedMeshesDedupMapping.csv`（原 `bakedmeshes-dedup-mapping.csv`）和
> `BakedMeshesDedupComponents.txt`（原 `bakedmeshes-dedup-components.log`，改 `.txt` 是因为工程 `.gitignore` 忽略 `*.log`），两者内容未改，路径同样是旧的。
