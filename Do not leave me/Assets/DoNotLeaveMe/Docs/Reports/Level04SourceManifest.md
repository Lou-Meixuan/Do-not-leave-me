...
- The two `PlayerSystem` player renderers were excluded because Persistent remains the sole owner of the formal human/dog pair.
...
- Unity clears Selection when the editor switches from the source scene to Level_04; verification is intentionally performed before that switch so it never relies on names or hierarchy afterward.
...
- `Level_04.unity` instantiates `L04_Content` under `Level04ContentRoot` and contains no player actors.
...
- `Level_04` is registered in Build Settings.
- Persistent `Initial Level Scene` is intentionally `Level_04` for the active fourth-level test session.
- Direct `Persistent -> Level_04` Play Mode verification loads exactly one grounded human/dog actor pair at the Level 4 entry anchors.
- Level_04 contains `266` Renderer components, `7` scene-owned Collider components, and no scene-local Camera. The persistent FormalMainCamera remains the only runtime main camera.
...
- Level_04 now contains `119` Collider components: the original floor/boundary foundation plus broad static coverage.
...
- Direct verification was run through `Persistent -> Level_04`, which spawned exactly one grounded human/dog pair. Directly pressing Play while Level_04 alone is open intentionally has no actors because players are persistent-scene owned.
