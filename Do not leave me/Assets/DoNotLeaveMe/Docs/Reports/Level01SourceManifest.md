...
The formal assembly is now instantiated into `Level_01` as `Prefabs/L01_Content.prefab`. It contains the selected art grouped as `Architecture`, `SetDressing`, and `GameplayVisuals`; the Level 1 scene retains only the prefab instance and level configuration roots.
