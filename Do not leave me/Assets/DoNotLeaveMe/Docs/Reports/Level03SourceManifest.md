...
Source scene: `Assets/_Archive/Scenes_SuperBreadManEra/Test/superbreadman 1.unity`
...
- The content Prefab is instantiated below `Level03ContentRoot` in `Level_03.unity`.
...
- The art Prefab contains no source runtime collision, navigation, character, checkpoint, exit, trigger, or mechanic components. Level_03 separately owns its traversal anchors and foundational collision.
...
- The rebuilt layout preserves the source objects' world positions, rotations, and scales, and the formal player pair now remains visible and grounded when Level 3 loads through Persistent.
...
- Level_03 now renders 536 objects including the persistent scene camera/light and scene-owned visualization; the rebuilt `L03_Content` contains 331 broad-scan visual children.
...
- Add `Level_03` to Build Settings before using formal additive loading.
- Open `Persistent.unity`, select `FormalGameFlow`, set `Initial Level Scene` to the exact name `Level_03`, then enter Play Mode.
- Direct-entry verification loads `Persistent` and `Level_03`, creates exactly one human/dog actor pair, and leaves both actors grounded on `Floor_WestNorth`.
- Restore `Initial Level Scene` to `Level_01` after direct Level 3 testing unless a temporary Level 3 test session is intentionally active.
...
- `Level_03` now contains `186` Collider components: the original `25` traversal-foundation colliders plus the new broad coverage proxies.
...
- Direct Play Mode test temporarily set `Persistent` to `Level_03`, loaded `Persistent` plus `Level_03`, and created exactly one grounded human/dog actor pair with Rigidbody components. The dog advanced along the approved corridor through the existing movement API without scene-collider blockage.
...
- `Persistent` was restored to `Level_01` after the direct test. Unity Console reported no errors or warnings.
...
- Direct `Persistent -> Level_03` verification spawned exactly one grounded human/dog pair at the Pad-adjacent anchors. Persistent was restored to `Level_01` afterward.
...
- `Level_03/Main Camera` is retained for reference but its Camera component is disabled and its tag is `Untagged`.
- Direct `Persistent -> Level_03` verification now reports exactly one enabled `MainCamera`: `FormalMainCamera` in Persistent, with CameraFollow targeting `FormalHumanActor`.
- Persistent was restored to `Level_01` after verification.
...
- Level_03 now has `92` enabled Collider components: `67` broad fixed-obstacle proxies, `24` dedicated traversal colliders, and the checkpoint trigger.
...
- Direct `Persistent -> Level_03` Play Mode verification still creates exactly one grounded human/dog pair at the Pad-adjacent anchors.
- `Persistent.initialLevelScene` remains `Level_03` for the active Level 3 test session.
