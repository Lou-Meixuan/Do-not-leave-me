...
public class MonsterPatrol : MonoBehaviour
{
    ...
    bool HasLineOfSight(Transform target)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (hit.GetComponentInParent<PlayerActor>() != null)
                continue;
    ...
    private PlayerActor executionTarget;
    ...
    void Start()
    {
        ...
        if (level != null)
            level.RegisterTemporaryState(new MonsterResetState(this));
    }
    ...
    void ResolveSafeZones()
    {
        if (safeZones != null && safeZones.Length > 0)
            return;

        ActuatorTrigger[] triggers = FindObjectsOfType<ActuatorTrigger>(true);
    ...
    void HandleFootsteps()
    {
        ...
        if (!footstepTracker.Tick(transform.position, requiredDistance,
            GameplayState.CanSimulate && currentState != State.Attack, Time.deltaTime))
            return;
    ...
    void CheckForPlayers()
    {
        ...
        if (PlayerActors.Instance != null)
        {
            if (TryCatch(PlayerActors.Instance.Human != null ? PlayerActors.Instance.Human.transform : null)) return;
            if (TryCatch(PlayerActors.Instance.Dog != null ? PlayerActors.Instance.Dog.transform : null)) return;
        }
        ...
        {
            PlayerActor formalPlayer = LevelActors.ResolvePlayer(hit);
    ...
    void BeginAttack(Transform target)
    {
        ...
        executionTarget = target.GetComponentInParent<PlayerActor>();
    ...
    void ExecuteCatch()
    {
        ...
        if (DeathScreen.Trigger(DeathScreen.DeathCause.Caught))
            return;
        ...
        if (flow != null)
            ...
            Debug.LogError("[MonsterPatrol] Monster capture requires GameFlowController for shared recovery.", this);
    }
    ...
    class MonsterResetState : ILevelTemporaryState
    {
        readonly MonsterPatrol patrol;

        public MonsterResetState(MonsterPatrol patrol)
        {
            this.patrol = patrol;
        }
