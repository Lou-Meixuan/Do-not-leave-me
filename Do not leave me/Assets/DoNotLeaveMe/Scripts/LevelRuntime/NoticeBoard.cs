...
/// 弹图复用 Persistent 场景里那个 TutorialPopup 面板（同一套翻页/关闭/暂停逻辑），
/// 但不写 PlayerPrefs，所以公告牌可以反复阅读，不是只能看一次。
///
/// 按键不是本脚本自己抢的：F 由 PlayerControl 统一分发，
/// 挂着箱子的时候 F 仍然是「松开箱子」，只有空手站在牌子前才算阅读。
///
/// 摆放位置：牌子模型下面挂一个空物体，或者直接挂在牌子根节点上，
/// readRadius 调到玩家会走到的范围（默认 3 米）。
/// </summary>
[AddComponentMenu("DoNotLeaveMe/Notice Board")]
public class NoticeBoard : MonoBehaviour
{
    ...
    private static readonly List<NoticeBoard> boards = new List<NoticeBoard>();
    ...
    void Update()
    {
        ...
        bool inside = InRange(ActorOf(PlayerActor.ActorRole.Human))
                   || InRange(ActorOf(PlayerActor.ActorRole.Dog));
    ...
    void ShowPrompt()
    {
        ...
        if (hud == null)
        {
            Debug.LogWarning("[NoticeBoard] 场景里找不到 HUDController，提示弹不出来：" + name, this);
            return;
        }
    ...
    /// <summary>
    /// PlayerControl 收到 F 时转发过来。返回 true 表示这次 F 被公告牌吃掉了。
    /// 附近有多块牌子时读最近的那一块。
    /// </summary>
    public static bool TryRead(PlayerActor actor)
    {
        if (actor == null || actor.IsExecutionLocked || TutorialPopup.IsShowing)
            return false;

        NoticeBoard nearest = null;
        float nearestDistance = float.MaxValue;

        for (int i = 0; i < boards.Count; i++)
        {
            NoticeBoard board = boards[i];
    ...
    bool Read(PlayerActor actor)
    {
        ...
        if (popup == null)
        {
            Debug.LogWarning("[NoticeBoard] 找不到 TutorialPopup（在 Persistent 场景里），公告弹不出来。", this);
            return false;
        }
    ...
    static PlayerActor ActorOf(PlayerActor.ActorRole role)
    {
        PlayerActors actors = PlayerActors.Instance;
        if (actors == null)
            return null;

        return role == PlayerActor.ActorRole.Dog ? actors.Dog : actors.Human;
    }

    bool InRange(PlayerActor actor)
    {
