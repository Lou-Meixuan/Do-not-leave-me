...
public class PlayerActor : MonoBehaviour
{
    ...
    void HandleFootsteps()
    {
        ...
        {
            Debug.LogWarning($"PlayerActor: {role} Footstep Event is not assigned.", this);
    ...
    void PlayAnimationForState()
    {
        ...
        {
            Debug.LogWarning("[PlayerActor] " + name + " missing animation state " + stateName);
            return;
        }

        animator.CrossFadeInFixedTime(resolvedState, blendTime, 0, 0f);
        //Debug.Log("[PlayerActor] " + name + " play animation " + resolvedState + " for " + state);
    }
