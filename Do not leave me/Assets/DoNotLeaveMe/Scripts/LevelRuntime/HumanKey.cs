...
/// RouteAdvanceTrigger。
/// </summary>
public class HumanKey : MonoBehaviour, ILevelTemporaryState
{
    [Header("这把钥匙开哪扇门")]
    ...
    [Tooltip("勾上=捡到钥匙那一刻门直接开。\n" +
             "取消=只是把钥匙记为“已获得”，玩家还得走到门口按 E 才开门（需要门上挂 DoorInteraction）。")]
    ...
    /// <summary>钥匙是否已被捡起。门上的 DoorInteraction 读这个来决定能不能开。</summary>
    ...
    {
        if (collected || !LevelActors.IsHuman(other))
            return;

        PlayerActor human = LevelActors.ResolvePlayer(other);
        pickupEmitter = human != null ? human.gameObject : other.gameObject;

        if (requireInteractToCollect)
        {
            humanInRange = true;
            if (!string.IsNullOrEmpty(promptPickup) && HUDController.Instance != null)
                HUDController.Instance.ShowHint(promptPickup);
            return;
        }
    ...
    void Collect()
    {
        ...
        {
            Debug.LogWarning("[HumanKey] Play_Key_Pickup is not assigned.", this);
        ...
        // DoorInteraction 读 IsCollected 来判断的，这里空着完全正常。
        if (openDoorsImmediately && !hasDoors)
            Debug.LogWarning("[HumanKey] 勾了 Open Doors Immediately 但 Doors To Unlock 是空的，" +
                             "捡到钥匙不会开任何门。", this);

        if (!openDoorsImmediately && HUDController.Instance != null)
            HUDController.Instance.ShowHint("拿到钥匙了。去门那边按 E 开门。");

        // 捡钥匙【绝对不】推进关卡。过关只能由门后面的
        // RouteAdvanceTrigger 触发——拿到钥匙不等于通关。
