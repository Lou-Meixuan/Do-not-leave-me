...
/// </summary>
[AddComponentMenu("DoNotLeaveMe/Level Intro Trigger")]
    ...
    void Update()
    {
        ...
        if (!InRange(actors.Human.transform))
        {
            if (Time.frameCount % 120 == 0)
                TutorialPopup.Trace("出生点触发区：人还没进范围，人在 " +
        ...
        {
            Debug.LogWarning("[LevelIntroTrigger] 找不到 TutorialPopup，弹不了。", this);
            return;
        }

        TutorialPopup.Trace("出生点触发区命中（" + gameObject.scene.name + "）");

        if (logWhenTriggered)
            Debug.Log("[LevelIntroTrigger] " + gameObject.scene.name + " 出生点触发区已命中。", this);
