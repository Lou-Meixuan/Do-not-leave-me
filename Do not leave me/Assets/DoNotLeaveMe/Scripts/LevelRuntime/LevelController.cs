...
public class LevelController : MonoBehaviour
{
    ...
    private readonly List<ILevelTemporaryState> temporaryStates = new List<ILevelTemporaryState>();
    ...
    public bool PlacePlayerAtRespawnAnchor(PlayerActor actor, Transform anchor)
    {
    ...
    public void RegisterTemporaryState(ILevelTemporaryState state)
    {
    ...
    public void SetCheckpoint(Transform humanAnchor, Transform dogAnchor)
    {
        ...
        if (!hasCheckpoint)
            Debug.LogError($"[LevelController] {levelId} checkpoint has no complete human/dog respawn-anchor pair.", this);
    }
    ...
    public bool RequestRecovery()
    {
        ...
        try
        {
            ...
            foreach (ILevelTemporaryState state in temporaryStates)
    ...
    void CollectSceneTemporaryStates()
    {
        ...
        foreach (GameObject root in scene.GetRootGameObjects())
        {
            ...
            {
                ILevelTemporaryState state = behaviour as ILevelTemporaryState;
                if (state == null || temporaryStates.Contains(state))
                    continue;
        ...
        if (added > 0)
            Debug.Log($"[LevelController] {levelId} 复位时补登记了 {added} 个漏注册的机关。", this);
    }
    ...
    bool PlacePlayers(Transform humanAnchor, Transform dogAnchor)
    {
        ...
        {
            Debug.LogError($"[LevelController] {levelId} cannot place players because the shared actor pair is unavailable.", this);
            return false;
        }
    ...
    bool TryResolveGroundPosition(Transform anchor, PlayerActor actor, out Vector3 position)
    {
        position = default;
        if (anchor == null || actor == null)
        {
            Debug.LogError($"[LevelController] {levelId} has a missing respawn anchor or actor.", this);
            return false;
        }
        ...
        foreach (RaycastHit hit in hits)
        {
            ...
            if (collider == null || collider.isTrigger || !collider.enabled ||
                collider.GetComponentInParent<PlayerActor>() != null)
                continue;
        ...
        Debug.LogError($"[LevelController] {levelId}/{anchor.name} has no valid ground below its XZ position; player placement was refused.", anchor);
        return false;
    }
}
