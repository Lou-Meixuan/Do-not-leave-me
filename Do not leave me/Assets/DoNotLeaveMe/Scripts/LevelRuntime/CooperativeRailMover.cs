...
public class CooperativeRailMover : MonoBehaviour, ILevelTemporaryState, IPushMover
{
    [System.Serializable]
    ...
    public bool IsAttached(PlayerActor actor) => actor != null && actor == human;
    ...
    public bool TryEngage(PlayerActor actor)
    {
        if (!enabled)
            return false;
        ...
        {
            Debug.LogWarning($"[CooperativeRailMover] F engage rejected: actor={(actor != null ? actor.name : "null")}, alreadyAttached={IsAttached(actor)}");
            return false;
        }
        ...
        if (group < 0)
        {
            Debug.LogWarning($"[CooperativeRailMover] F engage rejected: actor={actor.name}, actorPosition={actor.transform.position}, cratePosition={transform.position}, no matching interaction point within range={interactionRange:F2}");
            return false;
        }

        if (!CanUseGroup(actor, group))
        {
            Debug.LogWarning($"[CooperativeRailMover] F engage rejected: actor={actor.name}, group={group}, role={actor.Role}, group unavailable");
            return false;
        }

        if (actor.Role != PlayerActor.ActorRole.Human)
        {
            Debug.LogWarning($"[CooperativeRailMover] F engage rejected: actor={actor.name}, role={actor.Role}; only Human can attach");
            return false;
        }
        ...
        Debug.Log($"[CooperativeRailMover] F engage succeeded: actor={actor.name}, group={humanGroup}, movementAxis={(movementAxis == 0 ? "X" : "Z")}, origin={movementOrigin}");
        return true;
    }
    ...
    void Update()
    {
        ...
        if (pushAudioPlaying && !GameplayState.CanSimulate)
    ...
    {
        moving = moving && GameplayState.CanSimulate;
        ...
        if (moving)
        {
            ...
            {
                Debug.LogWarning("[CooperativeRailMover] Play_Crate_Push is not assigned.", this);
    ...
    void StopPushAudioImmediate()
    {
        ...
        else
        {
            ...
            {
                Debug.LogWarning("[CooperativeRailMover] Stop_Crate_Push is not assigned; stopped the loop without its release tail.", this);
    ...
    void IgnoreMoverCollision(PlayerActor actor, bool ignore)
    {
    ...
    Vector3 GetGroupPoint(int group, PlayerActor.ActorRole role)
    {
    ...
    int FindMatchingGroup(PlayerActor actor)
    {
        Vector3 actorOffset = actor.transform.position - transform.position;
        actorOffset.y = 0f;

        Debug.Log($"[CooperativeRailMover] F matching start: actorOffset={actorOffset}, actorFlatDistance={actorOffset.magnitude:F3}, sideTolerance={sideTolerance:F3}, interactionRange={interactionRange:F3}");
        ...
        for (int group = 0; group < 4; group++)
        {
            ...
            if (points == null)
            {
                Debug.Log($"[CooperativeRailMover] F matching group={group}: group=null");
                continue;
            }
            ...
            {
                Debug.Log($"[CooperativeRailMover] F matching group={group}: centerOffset={groupOffset}, invalid center direction");
                continue;
            }
            ...
            Debug.Log($"[CooperativeRailMover] F matching group={group}: point={pointPosition}, centerOffset={groupOffset}, pointDistance={pointDistance:F3}, dot={dot:F3}, sidePass={dot >= sideTolerance}, rangePass={pointDistance <= interactionRange}");
            if (dot > bestDot)
            {
                bestDot = dot;
                bestGroup = group;
        ...
        {
            Debug.Log($"[CooperativeRailMover] F matching rejected by side: bestGroup={bestGroup}, bestDot={bestDot:F3}, sideTolerance={sideTolerance:F3}");
            return -1;
        }
        ...
        Debug.Log($"[CooperativeRailMover] F matching selected: group={bestGroup}, point={bestPoint}, distance={bestDistance:F3}, range={interactionRange:F3}, inRange={inRange}");
        return inRange ? bestGroup : -1;
    }

    bool CanUseGroup(PlayerActor actor, int group)
    {
        if (actor.Role == PlayerActor.ActorRole.Human && humanGroup >= 0)
            return false;
        return actor.Role == PlayerActor.ActorRole.Human;
    }
