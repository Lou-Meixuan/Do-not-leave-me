...
/// 踏板被踩下的下沉反馈动画。
///
/// 挂在踏板根节点上（和 ActuatorTrigger 同一个物体）。踏板一旦触发，
...
/// 所以直接加组件就能用，不用手动拖引用。
/// </summary>
[RequireComponent(typeof(ActuatorTrigger))]
[AddComponentMenu("DoNotLeaveMe/Pedal Press")]
public class PedalPress : MonoBehaviour
{
    [Tooltip("看哪个触发器的状态。留空 = 用本物体上的 ActuatorTrigger")]
    [SerializeField] private ActuatorTrigger trigger;
    ...
    {
        if (trigger == null)
            trigger = GetComponent<ActuatorTrigger>();
        ...
        {
            Debug.LogWarning("[PedalPress] " + name +
