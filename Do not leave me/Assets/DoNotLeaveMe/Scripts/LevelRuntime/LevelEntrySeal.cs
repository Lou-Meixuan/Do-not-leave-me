...
///
/// 为什么需要它：过关点（LevelCheckpoint）是摆在【上一关】场景里的，
...
[AddComponentMenu("DoNotLeaveMe/Level Entry Seal")]
public class LevelEntrySeal : MonoBehaviour
{
    [Header("条件")]
    ...
    [Tooltip("人一踏进这个入口区，就弹一次本关介绍图。\n" +
             "图配在 Persistent 的 FormalUI / Tutorial Popup 的 Level Intro Pages 上，" +
    ...
    private readonly HashSet<PlayerActor> inside = new HashSet<PlayerActor>();
    ...
    {
        PlayerActor actor = LevelActors.ResolvePlayer(other);
        if (actor == null)
            return;
        ...
        {
            TutorialPopup.Trace("入口封关区被踩到（" + gameObject.scene.name + "），popup=" +
    ...
    void TrySeal()
    {
        ...
        if (flow.ConfirmPreloadedPhysicalArrival(gameObject.scene.name, this))
        {
            ...
            sealDone = true;
            if (logWhenSealed)
                Debug.Log($"[LevelEntrySeal] {gameObject.scene.name} 实体进关已确认，前关已按本关策略处理。", this);
            return;
        }
        ...
        if (logWhenSealed)
            Debug.Log($"[LevelEntrySeal] {gameObject.scene.name} 入口已封，上一关卸载。", this);
    }
    ...
    bool ConditionMet()
    {
        ...
        bool hasHuman = false;
        bool hasDog = false;
        foreach (PlayerActor actor in inside)
        {
