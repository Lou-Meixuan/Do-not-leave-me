...
/// <summary>
/// 统一地面工具集（Tools/DoNotLeaveMe/Ground/）。全部为小工具：
/// 1) 从选中渲染器建/重 fitting 地面体  2) 禁用 NavGround 层旧地面碰撞(可回滚)
/// 3) 审计覆盖洞/高度错位  4) 多选同步表面高度。
/// </summary>
public static class GroundTools
{
    const string JournalFolder = "Assets/DoNotLeaveMe/Docs/Reports/GroundJournals";
    const int NavGroundLayer = 6; // ProjectSettings/TagManager: NavGround

    // ---------------------------------------------------------------- 1. Volume From Selection

    [MenuItem("Tools/DoNotLeaveMe/Ground/Create Or Fit Volume From Selection")]
    static void CreateOrFitVolumeFromSelection()
    {
        GroundVolume target = Selection.gameObjects
            .Select(go => go.GetComponent<GroundVolume>())
            .FirstOrDefault(v => v != null);

        List<Renderer> renderers = new List<Renderer>();
        foreach (GameObject go in Selection.gameObjects)
        {
            if (go.GetComponent<GroundVolume>() != null)
                continue;
        ...
        {
            Debug.LogWarning("[GroundTools] 选中内容里没有可用的 Renderer（或只选了地面体本身）。");
            return;
        }
        ...
        if (target == null)
        {
            GameObject go = new GameObject("GroundVolume");
            Undo.RegisterCreatedObjectUndo(go, "Create GroundVolume");
            go.layer = NavGroundLayer;
            target = go.AddComponent<GroundVolume>();
            target.Box.size = new Vector3(1f, target.Thickness, 1f);
        }
        else
        {
            Undo.RecordObject(target.transform, "Fit GroundVolume");
            Undo.RecordObject(target.Box, "Fit GroundVolume");
        }
        ...
        Debug.Log($"[GroundTools] volume at {target.transform.position}, footprint {size.x:F1}x{size.z:F1}, top={target.TopHeight:F3}");
    }

    // ---------------------------------------------------------------- 2. Disable NavGround colliders (+ rollback)

    [MenuItem("Tools/DoNotLeaveMe/Ground/Disable NavGround Colliders (Except Volumes)")]
    static void DisableNavGroundColliders()
    {
        ...
        using (StreamWriter writer = new StreamWriter(journalPath, false))
        {
            ...
            Debug.Log($"[GroundTools] disabled {disabled} NavGround collider(s). journal={journalPath}");
        }
    }

    [MenuItem("Tools/DoNotLeaveMe/Ground/Rollback Last Disable Pass")]
    static void RollbackLastDisablePass()
    {
        ...
        if (journalPath == null)
        {
            Debug.LogWarning("[GroundTools] 没有可回滚的 disable 日志。");
            return;
        }
        ...
        Debug.Log($"[GroundTools] rollback finished: {restored} collider(s) re-enabled from {Path.GetFileName(journalPath)}.");
    }
    ...
    static int TryDisable(Collider collider, string sceneName, StreamWriter writer)
    {
        if (collider == null || !collider.enabled || collider.gameObject.layer != NavGroundLayer)
            return 0;
        if (collider.GetComponentInParent<GroundVolume>() != null)
            return 0;
    ...
    [MenuItem("Tools/DoNotLeaveMe/Ground/Audit Ground Coverage")]
    static void AuditGroundCoverage()
    {
        List<GroundVolume> volumes = Object.FindObjectsOfType<GroundVolume>(false).ToList();
        if (volumes.Count == 0)
        {
            Debug.LogWarning("[GroundTools] 场景里没有 GroundVolume，无法审计。");
            return;
        }
            ...
            foreach (GameObject root in scene.GetRootGameObjects())
            {
                foreach (Renderer renderer in root.GetComponentsInChildren<Renderer>(false))
                {
                    if (!visited.Add(renderer) || renderer is ParticleSystemRenderer)
                        continue;
                    if (renderer.GetComponentInParent<PlayerActor>() != null)
                        continue;
        ...
        sb.AppendLine($"[GroundTools] audit: volumes={volumes.Count} referenceTop={referenceTop:F3} holes={holes.Count} misaligned={misaligned.Count}");
    ...
    static bool CoversXZ(GroundVolume volume, Bounds b)
    {
    ...
    [MenuItem("Tools/DoNotLeaveMe/Ground/Copy Top Height To Selected Volumes")]
    static void CopyTopHeightToSelection()
    {
        GroundVolume[] selected = Selection.gameObjects
            .Select(go => go.GetComponent<GroundVolume>())
            .Where(v => v != null)
            .ToArray();
        if (selected.Length < 2)
        {
            Debug.LogWarning("[GroundTools] 请选中两个及以上带 GroundVolume 的物体。");
            return;
        }
        ...
        Debug.Log($"[GroundTools] copied topHeight {source:F3} to {applied} volume(s).");
    }
