...
///   3. 菜单 Tools / DoNotLeaveMe / 贴纸 / 把选中的图贴到当前视角中心。
///
/// 图会生在视图正中间、自动转向面对你的镜头，然后自己微调位置就行。
/// 图的导入设置不是 Sprite 的话工具会顺手改掉，不用手动去 Inspector 点。
///
/// 生成的结构：
///   Sticker_xxx   (WallSticker，靠近弹字，Hint Message 自己填)
...
{
    private const string HintArtFolder = "Assets/Art/UI/LevelHints";
    ...
    [MenuItem("Tools/DoNotLeaveMe/贴纸/把选中的图贴到当前视角中心 %#j")]
    public static void PlaceSelectedSticker()
    {
        ...
        Undo.AddComponent<WallSticker>(sticker);
        ...
        Debug.Log("[Sticker] 已贴上 " + sticker.name + "（场景 " + scene.name + "，位置 " +
                  ...
                  "在 Inspector 的 Wall Sticker / Hint Message 里填靠近时要弹的文字，" +
                  "留空就是纯装饰。摆好之后 Ctrl+S。", sticker);
    }

    [MenuItem("Tools/DoNotLeaveMe/贴纸/让选中的贴纸重新对齐当前视角")]
    public static void RealignSelectedSticker()
    {
        GameObject selected = Selection.activeGameObject;
        WallSticker sticker = selected != null ? selected.GetComponentInParent<WallSticker>() : null;
        if (sticker == null)
        {
            Debug.LogWarning("[Sticker] 先在 Hierarchy 里选中一个贴纸（带 Wall Sticker 的那个物体）。");
            return;
        }
    ...
    [MenuItem("Tools/DoNotLeaveMe/贴纸/把「UI/LevelHints」整个文件夹转成 Sprite")]
