...
/// <summary>
/// 一键在 Persistent 场景里搭出正式关卡的 UI 层（HUD + 暂停菜单 + 设置面板）。
///
/// 为什么用编辑器脚本而不是直接改场景文件：UI 组件（Slider / Button / Canvas）
/// 的序列化结构很啰嗦，手写 YAML 极容易错。让 Unity 自己建更稳。
///
/// 用法：打开 Persistent 场景 -> 菜单 Tools / DoNotLeaveMe / 构建正式关 UI。
...
public static class UIBuilder
{
    ...
    const string SettingsPrefabPath = "Assets/DoNotLeaveMe/Prefabs/SettingsPanel.prefab";

    [MenuItem("Tools/DoNotLeaveMe/构建正式关 UI (Persistent)")]
    public static void Build()
    {
        ...
        if (scene.name != "Persistent")
        {
            EditorUtility.DisplayDialog("场景不对",
                "请先打开 Assets/DoNotLeaveMe/Levels/Persistent.unity 再执行。\n当前打开的是：" + scene.name,
                "知道了");
            return;
        }
        ...
        root.AddComponent<AnxietyOverlay>();
        ...
        Debug.Log("[UIBuilder] FormalUI 构建完成。记得 Ctrl+S 保存场景。");
    }
    ...
    static void BuildDeathScreen(Transform parent, GameObject controllerHost)
    {
        ...
        var screen = controllerHost.AddComponent<DeathScreen>();
        screen.root = death;
        screen.group = group;
    ...
    static void BuildTutorialPopup(Transform parent, GameObject controllerHost)
    {
        ...
        var tutorial = controllerHost.AddComponent<TutorialPopup>();
        tutorial.root = popup;
        ...
        tutorial.checkpointPrefKey = TutorialPopup.CheckpointPrefKeyDefault;
        tutorial.checkpointDelaySeconds = 1f;
        tutorial.humanLevelIntroPages = LoadLevel045Sprites();
        tutorial.levelIntroScene = "Level_04B";
        tutorial.levelIntroPrefKey = TutorialPopup.Level045PrefKeyDefault;
        tutorial.levelIntroDelaySeconds = 1f;
        tutorial.triggerLevelScene = "Level_01";
    ...
    /// </summary>
    [MenuItem("Tools/DoNotLeaveMe/挂上额外教程图（地毯 + 4.5 关）")]
    public static void AttachSaveAreaTutorialSprite()
    {
        var popup = Object.FindObjectOfType<TutorialPopup>();
        if (popup == null)
        {
            EditorUtility.DisplayDialog("找不到 Tutorial Popup",
                "请先打开 Assets/DoNotLeaveMe/Levels/Persistent.unity 再执行。", "知道了");
            return;
        }
        ...
        popup.checkpointPrefKey = TutorialPopup.CheckpointPrefKeyDefault;
        ...
        if (string.IsNullOrEmpty(popup.levelIntroScene))
            popup.levelIntroScene = "Level_04B";
        popup.levelIntroPrefKey = TutorialPopup.Level045PrefKeyDefault;
        ...
        Debug.Log("[UIBuilder] 额外教程图已挂上（地毯 2 张 + 4.5 关 1 张），记得 Ctrl+S 保存场景。", popup);
    }

    [MenuItem("Tools/DoNotLeaveMe/重置玩法介绍（下次再弹一遍）")]
    public static void ResetTutorialFlag()
    {
        PlayerPrefs.DeleteKey(TutorialPopup.PrefKey);
        PlayerPrefs.DeleteKey(TutorialPopup.CheckpointPrefKeyDefault);
        PlayerPrefs.DeleteKey(TutorialPopup.Level045PrefKeyDefault);
        PlayerPrefs.Save();
        Debug.Log("[UIBuilder] 玩法介绍 + 存档地毯介绍 + 4.5 关介绍已重置，下次会再弹一遍。");
    }
    ...
    static GameObject BuildSettingsPanel(Transform parent)
    {
        ...
        if (prefab == null)
        {
            Debug.LogWarning("[UIBuilder] 找不到 " + SettingsPrefabPath + "，设置面板跳过。");
            return null;
        }
    ...
    static void EnsureEventSystem()
    {
        ...
        Debug.Log("[UIBuilder] 场景里没有 EventSystem，已补上（没有它 UI 点不动）。");
    }
    ...
    static void EnsureAnxietyPostFX()
    {
        ...
        if (volume == null)
        {
            Debug.LogWarning("[UIBuilder] 场景里没有 Volume，色差效果跳过。");
            return;
        }

        volume.gameObject.AddComponent<AnxietyPostFX>();
        Debug.Log("[UIBuilder] 已把 AnxietyPostFX 挂到 " + volume.gameObject.name + " 上。");
    }

    static void EnsureAnxietyState()
    {
        if (Object.FindObjectOfType<AnxietyState>() != null)
            return;

        var go = new GameObject("FormalAnxiety");
        go.AddComponent<AnxietyState>();
        Debug.Log("[UIBuilder] 场景里没有 AnxietyState，已补上。" +
    ...
    static Font BuiltinFont()
    {
        ...
        if (f == null)
            Debug.LogWarning("[UIBuilder] 没拿到内置字体，文字可能不显示。手动给 Text 指定一个字体。");
        return f;
    }
    ...
    static Sprite LoadSprite(string path)
    {
        ...
        if (sp == null)
            Debug.LogWarning("[UIBuilder] 加载不到图：" + path + "（检查路径，或者这张图的 Texture Type 不是 Sprite）");
        return sp;
    }
}
