...
/// 在第五关末尾摆一块「通关触发区」。
///
/// 玩家（或人和狗）走进这块区域 -> RouteAdvanceTrigger 请求推进路线 ->
/// 第五关后面没有下一关了 -> GameFlowController 判定通关 ->
/// 切到结尾动画场景 Cutscene_End -> 动画播完自动回主菜单 Start。
///
/// 用法：打开 Level_05，Scene 视图把镜头对准出口那块地，点菜单。
/// 触发区大小默认 6 x 4 x 6，自己在 Inspector 里改 Box Collider 的 Size。
///
/// 只标脏不自动存盘，摆好之后 Ctrl+S。
/// </summary>
public static class RouteExitBuilder
{
    private const string Level05ScenePath = "Assets/DoNotLeaveMe/Levels/Level_05.unity";
    private const string ExitObjectName = "L05_RouteExitTrigger";

    [MenuItem("Tools/DoNotLeaveMe/第五关/在当前视角中心放通关触发区")]
    public static void BuildRouteExitTrigger()
    {
        ...
        if (!scene.isLoaded)
        {
            EditorUtility.DisplayDialog(
                "通关触发区",
                "先打开第五关场景（Level_05），把 Scene 视图对准出口那块地，再点这个菜单。",
                "知道了");
            return;
        }
        ...
        Undo.AddComponent<RouteAdvanceTrigger>(exit);
