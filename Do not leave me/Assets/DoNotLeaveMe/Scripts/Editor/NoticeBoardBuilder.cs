...
///
/// 用法（两步）：
///   1. 打开 Level_03 场景，在 Scene 视图里把镜头对准想贴牌子的那面墙
///      （牌子会生在视图正中间，并自动转向面对你的镜头）；
///   2. 点菜单 Tools / DoNotLeaveMe / 第三关 / 创建或刷新公告牌。
///
/// 生成的结构：
///   L03_NoticeBoard   (NoticeBoard，阅读范围球心就在这儿)
...
/// </summary>
public static class NoticeBoardBuilder
{
    private const string Level03ScenePath = "Assets/DoNotLeaveMe/Levels/Level_03.unity";
    private const string ArtFolder = "Assets/Art/UI/07_Level3NoticeBoard/";
    ...
    [MenuItem("Tools/DoNotLeaveMe/第三关/创建或刷新公告牌")]
    public static void BuildNoticeBoard()
    {
        ...
        NoticeBoard notice = board.GetComponent<NoticeBoard>();
        if (notice == null)
            notice = Undo.AddComponent<NoticeBoard>(board);
    ...
    [MenuItem("Tools/DoNotLeaveMe/第三关/把公告牌移到当前视角中心")]
