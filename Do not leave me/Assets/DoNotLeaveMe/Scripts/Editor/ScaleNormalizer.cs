...
{
    const string OutputFolder = "Assets/DoNotLeaveMe/Levels/BakedMeshes";
    const string JournalFolder = "Assets/DoNotLeaveMe/Levels/BakedMeshes/Journals";

    [MenuItem("Tools/DoNotLeaveMe/Normalize Scale On Selected Leaf Meshes")]
    static void NormalizeSelected()
    {
        ...
        Debug.Log($"[ScaleNormalizer] normalized {normalized} leaf mesh node(s).");
    }

    [MenuItem("Tools/DoNotLeaveMe/Rollback Last Scene Normalization")]
    static void RollbackActiveSceneMenu()
    {
        ...
        Debug.Log($"[ScaleNormalizer] rollback finished: {restored} node(s) restored.");
    }
    ...
    public static int RollbackScene(string sceneName)
    {
        ...
        foreach (string line in File.ReadAllLines(journalPath))
        {
            ...
            if (node == null)
            {
                Debug.LogWarning($"[ScaleNormalizer] rollback skipped missing node {fields.path}");
                continue;
            }
    ...
    static string PersistMesh(Mesh mesh, string sourceName, string nodeName)
    {
        ...
        {
            Debug.LogError($"[ScaleNormalizer] failed to save mesh asset {path}: {exception.Message}");
            return null;
        }
        return path;
    }
