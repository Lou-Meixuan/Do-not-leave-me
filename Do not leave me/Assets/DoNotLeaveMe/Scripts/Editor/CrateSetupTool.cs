...
/// Tools / DoNotLeaveMe / 可推箱 / 把选中的物体改造成可推箱。
///
/// 它会补齐 PushableCrate 需要的一整套东西：
///   - BoxCollider（没有就按网格自动贴合）
///   - Rigidbody（参数由 PushableCrate.Awake 接管，这里只保证存在）
///   - PushableCrate 组件
...
public static class CrateSetupTool
{
    ...
    public static void ConvertSelectionToCrate()
    {
        ...
        PushableCrate crate = target.GetComponent<PushableCrate>();
        if (crate == null)
            crate = Undo.AddComponent<PushableCrate>(target);
        ...
        so.FindProperty("axisMode").enumValueIndex = (int)PushableCrate.PushAxisMode.Free;
        ...
        Debug.Log("[可推箱] " + target.name + " 已经是可推箱了。\n" +
                  ...
                  "同场景里的 CrateDoorTrigger 会在箱子进入它的触发区时开门 —— " +
    ...
    {
        CrateDoorTrigger[] triggers = Object.FindObjectsOfType<CrateDoorTrigger>(true);
        if (triggers.Length == 0)
        {
            Debug.LogWarning("[可推箱] 这个场景里没有 CrateDoorTrigger —— 箱子推到哪儿都不会开门。", crate);
            return;
        }

        foreach (CrateDoorTrigger trigger in triggers)
        {
