...
///
/// 挂载位置：Persistent 场景的 FormalUI Canvas 上（由 UIBuilder 自动搭建）。
/// 触发时机：triggerLevelScene 这一关加载完、人和狗都就位的那一刻。
/// 记忆方式：PlayerPrefs，永久记住。想再看一遍走
/// 菜单 Tools / DoNotLeaveMe / 重置玩法介绍。
/// </summary>
[AddComponentMenu("DoNotLeaveMe/Tutorial Popup")]
public class TutorialPopup : MonoBehaviour
{
    ...
    [Tooltip("哪一关进去之后弹")]
    public string levelIntroScene = "Level_04B";
    ...
    public string triggerLevelScene = "Level_01";
    ...
    Sprite[] RolePages(Sprite[] humanContent, Sprite[] dogContent, string contentName)
    {
        ...
        {
            Debug.LogWarning("[TutorialPopup] " + contentName + " 没配 " +
    ...
    /// 这条是兜底轮询：走关卡入口封关区（LevelEntrySeal）那条路更早、更准。
    /// </summary>
    ...
    /// <summary>
    /// 本关介绍图。由关卡入口的 LevelEntrySeal（勾了 Show Level Intro Tutorial 的那个）调，
    ...
    void BeginLevelIntro(string reason)
    {
        ...
        if (rolePages == null)
        {
            Debug.LogWarning("[TutorialPopup] 该弹本关介绍了（" + reason +
                             "），但 Level Intro Pages 是空的，这次不弹。" +
                             "去 Persistent 场景跑一下 Tools/DoNotLeaveMe/挂上额外教程图（地毯 + 4.5 关），然后 Ctrl+S。", this);
            Trace("!! 该弹本关介绍了（" + reason + "），但 Level Intro Pages 是空的");
            return;
        }

        Debug.Log("[TutorialPopup] " + reason + "，" + levelIntroDelaySeconds + " 秒后弹本关介绍。", this);
    ...
    /// <summary>
    /// 存档地毯介绍。第二关踩到存档点时由 LevelCheckpoint 调，全流程只弹一次。
    ...
    public bool ShowCheckpointTutorial()
    {
        ...
        if (rolePages == null)
        {
            Debug.LogWarning("[TutorialPopup] 存档地毯介绍没配图（Checkpoint Pages 是空的），这次不弹。" +
                             "去 Persistent 场景跑一下 Tools/DoNotLeaveMe/挂上额外教程图（地毯 + 4.5 关）。", this);
            return false;
        }
