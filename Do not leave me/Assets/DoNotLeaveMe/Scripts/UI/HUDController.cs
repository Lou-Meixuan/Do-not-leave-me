...
/// AnxietyState + PlayerActors + PlayerControl，引用对不上。
///
/// 挂载位置：Persistent 场景里的 FormalHUD Canvas 根节点。
...
[AddComponentMenu("DoNotLeaveMe/HUD Controller")]
public class HUDController : MonoBehaviour
{
    ...
    [TextArea(2, 5)]
    public string humanControls = "WASD 移动   Space 跳跃   LeftShift 疾跑\nE 互动   F 推箱子   Tab 切换到小狗   B 阅读档案";
    [TextArea(2, 5)]
    public string dogControls = "WASD 移动   Space 跳跃   Tab 切换到人   B 阅读档案";
    ...
    {
        AnxietyState current = AnxietyState.Instance;
        if (current == anxiety)
            return;
    ...
    public void ShowHint(string message, float duration)
    {
        ...
        {
            Debug.LogWarning("[HUDController] hintText / hintGroup 没有赋值，提示无法显示：" + message);
            return;
        }
