...
/// - 数据源是 PlayerActors（正式关的人/狗），不是老的 PlayerManager。
/// - 挂在 Persistent 常驻场景上，关卡 additive 加载/卸载都不会丢。
///
/// 用法：
/// 1. 在 Persistent 场景里新建空物体 "FormalAnxiety"，挂本脚本。
/// 2. 表现层（进度条、后处理、音效）自己订阅 OnNormalizedChanged，或者轮询 Normalized。
/// </summary>
[AddComponentMenu("DoNotLeaveMe/Anxiety State")]
public class AnxietyState : MonoBehaviour
{
    public enum FullBehaviour
    {
        /// <summary>只发事件，由别人决定怎么处理</summary>
        None,
        /// <summary>通过 GameFlowController 请求当前关的统一重生。</summary>
        ResetLevel,
    }

    public static AnxietyState Instance { get; private set; }
    ...
    private PlayerActors cachedActors;
    ...
    /// <summary>
    /// 找到当前关卡里的 PlayerActors。关卡是 additive 加载的，
    ...
    private bool RefreshActors()
    {
        ...
        if (actors != cachedActors)
        {
            ...
            if (resetOnLevelChange && actors != null)
            {
                ResetAnxiety();
                if (logStateChanges)
                    Debug.Log("[AnxietyState] 检测到新关卡的角色，焦虑已重置。");
            }
        }
    ...
    private void EvaluateSeparation()
    {
        ...
        {
            if (logStateChanges)
                Debug.Log($"[AnxietyState] 分离状态 -> {IsSeparated}（距离 {ActorDistance:F2}）");
    ...
    private void HandleFull()
    {
        ...
        switch (onFull)
        {
            case FullBehaviour.ResetLevel:
                ...
                if (DeathScreen.Trigger(DeathScreen.DeathCause.Anxiety))
                {
                    ResetAnxiety();
                    break;
                }
                ...
                if (flow != null)
                    ...
                    Debug.LogWarning("[AnxietyState] 焦虑涨满，但场景里找不到 GameFlowController。");
                ResetAnxiety();
                break;
