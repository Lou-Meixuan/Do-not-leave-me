...
public sealed class FormalAsylumAudioRouter : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            var reference = Resources.Load<WwiseEventReference>("DoNotLeaveMe/AsylumAudio/Play_Asylum_" + names[i]);
    ...
    private void Scan()
    {
        ...
        for (int s = 0; s < SceneManager.sceneCount; s++)
        {
            ...
            if (!scene.isLoaded || !(scene.name.StartsWith("Level_", StringComparison.Ordinal) ||
                scene.name.StartsWith("SharedArt_", StringComparison.Ordinal))) continue;
