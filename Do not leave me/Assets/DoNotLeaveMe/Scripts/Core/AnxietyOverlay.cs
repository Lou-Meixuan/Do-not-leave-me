...
/// 这里把那部分逻辑原样搬过来，数据源换成 AnxietyState。
///
/// 挂载位置：Persistent 场景的 FormalUI Canvas 上（或任意常驻物体）。
/// 遮罩会自己建到 targetCanvas 底下，并且插在最前面几个 sibling，
/// 保证压在 HUD 下面、不挡暂停菜单。
///
/// 色差（眩晕）不在这里，那是 AnxietyPostFX 干的活。
/// </summary>
[AddComponentMenu("DoNotLeaveMe/Anxiety Overlay")]
public class AnxietyOverlay : MonoBehaviour
{
    [Header("挂载到哪个 Canvas")]
    ...
    void Start()
    {
        ...
        {
            Debug.LogWarning("[AnxietyOverlay] 找不到 Canvas，污渍和红晕都出不来。", this);
            enabled = false;
            return;
        }
    ...
    void LateUpdate()
    {
        float anxiety = AnxietyState.Instance != null
            ? Mathf.Clamp01(AnxietyState.Instance.Normalized)
            : 0f;
    ...
    void SetupDirtOverlay()
    {
        ...
        {
            Debug.LogWarning("[AnxietyOverlay] 污渍贴图没找到：Resources/"
        ...
        if (additive != null)
            ...
            Debug.LogWarning("[AnxietyOverlay] 找不到 DoNotLeaveMe/UI Additive shader，"
