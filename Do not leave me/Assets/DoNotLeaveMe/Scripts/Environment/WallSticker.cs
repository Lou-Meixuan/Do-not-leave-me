...
///
/// 和公告牌（NoticeBoard）的区别：
///   公告牌  = 走近提示「按 F 阅读」，按 F 才弹整页大图，一般只需要看一次；
///   贴纸    = 不用按键，走近就出文字，走远再走回来还会再出，随便看多少次。
///
/// 文字走的是 HUDController 的居中提示（和第一关那些引导同一套）。
/// hintMessage 留空 = 纯装饰，只有图，不出字。
///
/// 摆放：用菜单 Tools / DoNotLeaveMe / 贴纸 / 把选中的图贴到当前视角中心 生成，
/// 生成出来的结构是：
///   Sticker_xxx   (本脚本，触发范围球心在这儿)
///     └ Face      (SpriteRenderer，图本身)
/// </summary>
[AddComponentMenu("DoNotLeaveMe/Wall Sticker")]
public class WallSticker : MonoBehaviour
{
    ...
    void ShowHint()
    {
        ...
        if (hud == null)
        {
            Debug.LogWarning("[WallSticker] 场景里找不到 HUDController，文字弹不出来：" + name, this);
            return;
        }
    ...
    float FlatDistanceTo(PlayerActor actor)
    {
